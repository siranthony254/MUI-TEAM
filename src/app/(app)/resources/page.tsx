...
export default async function Resources({
  ...
  let query = supabase.from('resources').select('*').is('project_id', null).is('department_id', null).order('created_at', { ascending: false })
    ...
    <>
      ...
      <form className="mb-3 flex gap-2">
        ...
        <input name="q" defaultValue={q} placeholder="Search by title…" aria-label="Search resources"
          className="w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm text-neutral-900 outline-none focus:ring-2 focus:ring-amber-500 dark:border-white/15 dark:bg-[#0B1420] dark:text-neutral-100" />
        <button className="rounded-lg border border-neutral-300 bg-white px-4 text-sm font-medium text-neutral-700 hover:bg-neutral-50 dark:border-white/15 dark:bg-[#0B1420] dark:text-neutral-200 dark:hover:bg-white/10">Search</button>
      </form>
